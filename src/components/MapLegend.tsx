...
export default function MapLegend({ mapType }: MapLegendProps) {
  ...
  const gradiantStops = data.stops
    .map((stop) => `${stop.color} ${(stop.value / maxValue) * 100}%`)
    .join(", ");
  return (
    <div className="absolute top-4 right-4 z-1000 xs:w-96 w-48 rounded-xl shadow-lg p-4 bg-background/50 border border-accent/70 flex flex-col gap-3">
      <h3 className="text-sm font-semibold text-foreground">{data.title}</h3>
      <div
        className="w-full h-6 rounded-xl border border-accent/70"
        style={{
          background: `linear-gradient(to right, ${gradiantStops})`,
        }}
      ></div>
